...
OT_PROTOCOLS = {
    "Modbus": {
        "ports": [502],
        "signatures": [b'\x00\x00\x00\x00\x00\x06', b'\x00\x01\x00\x00\x00\x06'],
        "asset_type": "PLC/RTU",
        "vendor": None
    },
    "Siemens S7": {
        "ports": [102],
        "signatures": [b'\x03\x00\x00', b'\x32\x01\x00'],
        "asset_type": "Siemens PLC",
        "vendor": "Siemens"
    },
    "DNP3": {
        "ports": [20000],
        "signatures": [b'\x05\x64', b'\x05\x65'],
        "asset_type": "RTU/IED",
        "vendor": None
    },
    "BACnet": {
        "ports": [47808],
        "signatures": [b'\x81\x0a'],
        "asset_type": "Building Automation",
        "vendor": None
    },
    "EtherNet/IP": {
        "ports": [44818],
        "signatures": [b'\x65\x00\x04\x00'],
        "asset_type": "Rockwell PLC",
        "vendor": "Rockwell Automation"
    },
    "IEC 104": {
        "ports": [2404],
        "signatures": [b'\x68\x01', b'\x68\x04'],
        "asset_type": "SCADA RTU",
        "vendor": None
    },
    "OPC UA": {
        "ports": [4840],
        "signatures": [b'OPC', b'UA'],
        "asset_type": "OPC Server",
        "vendor": None
    },
    "CODESYS": {
        "ports": [2455],
        "signatures": [b'CODESYS'],
        "asset_type": "Industrial Controller",
        "vendor": None
    },
    "Profinet": {
        "ports": [34964, 34965],
        "signatures": [b'\xfe\xfe', b'\x01\x02'],
        "asset_type": "Profinet Device",
        "vendor": "Siemens"
    },
    "EtherCAT": {
        "ports": [34980],
        "signatures": [b'\x01\x00\x00\x00', b'\x02\x00\x00\x00'],
    ...
    "IEC 61850 GOOSE": {
        "ports": [],
        "signatures": [b'\x88\xb8'],  # Ethernet type 0x88B8
        "asset_type": "GOOSE Publisher",
        "vendor": None
    },
    "IEC 61850 SV": {
        "ports": [],
        "signatures": [b'\x88\xba'],  # Ethernet type 0x88BA
        "asset_type": "Sampled Values",
        "vendor": None
    }
}
...
def analyze_pcap(pcap_path, max_packets=10000):
    ...
    for pkt in packets:
        ...
        if Raw in pkt:
            ...
            if host_match:
                host = host_match.group(1).decode(errors='ignore')
                ip_data[dst_ip]["hostnames"].add(host)
            # HTTP User-Agent
